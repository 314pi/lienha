...
class WFToolsHelper {
	...
	function parseColors($file)
	{
		...
		if ($data) {
			if (preg_match_all('/@import url\(([^\)]+)\)/', $data, $matches)) {
				$template = self::getVeriTemplates();
	...
	function getVeriTemplateColors() 
	{
		...
		$templates 	= self::getVeriTemplates();		
	...
	function getOptions($params)
	{
		$options = array(
			...
			'colorpicker'	=> array(
				'template_colors' 	=> self::getVeriTemplateColors(),
