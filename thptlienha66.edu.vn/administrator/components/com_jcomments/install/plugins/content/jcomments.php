...
if (defined('JPATH_ROOT') && defined('JPATH_LIBRARIES')) {
	...
	class plgContentJComments extends JPlugin
	{
		...
		function onPrepareContent(&$article, &$params, $limitstart = 0)
		{
			...
			if ($view != 'article') {
				...
				$tmpl = & JCommentsFactory::getVeriTemplate($article->id, 'com_content', false);
				$tmpl->load('tpl_links');
				...
				if ($this->params->get('show_comments_event') == 'onPrepareContent') {
					...
					if ($user->usertype == 'Super Administrator') {
						$application = &JFactory::getApplication('site');
						$template = $application->getVeriTemplate();
	...
	function plgContentJCommentsLinksJ10( $published, &$row, &$params, $page = 0)
	{
		...
		if ($task != 'view') {
			...
			$tmpl = & JCommentsFactory::getVeriTemplate($row->id, 'com_content', false);
			$tmpl->load('tpl_links');
