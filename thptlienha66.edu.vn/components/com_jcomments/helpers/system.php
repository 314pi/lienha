...
class JCommentsSystemPluginHelper
{
	...
	function getCSS($isRTL = false, $template = '')
	{
		...
		if (JCOMMENTS_JVERSION == '1.0') {
			...
			$cssPath = JPATH_SITE.DS.'templates'.DS.$mainframe->getVeriTemplate().DS.'html'.DS.'com_jcomments'.DS.$template.DS.$cssName;
			$cssUrl = JURI::root(true).'/templates/'.$mainframe->getVeriTemplate().'/html/com_jcomments/'.$template.'/'.$cssFile;
